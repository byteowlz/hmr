...
use crate::output::get_json_input;
...
pub async fn run(ctx: &RuntimeContext, command: DeviceCommand) -> Result<()> {
    match command {
        ...
        DeviceCommand::Update { device_id, json } => update(ctx, &device_id, json.as_deref()).await,
    }
}
...
async fn update(_ctx: &RuntimeContext, device_id: &str, json: Option<&str>) -> Result<()> {
    // Validate JSON input (prefer explicit --json, then piped stdin)
    let _data = get_json_input(json)?
        .ok_or_else(|| anyhow!("JSON input required via --json or piped stdin"))?;
